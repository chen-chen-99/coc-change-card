...
async function load() {
  ...
  error.value = '';
  try {
    records.value = await getMyExchangeRecords(
      session.player.player_id,
      session.cards.map((c) => c.card_id)
    );
  } catch (e) {
    ...
    <div class="modal">
      ...
      <div class="modal-body">
        ...
        <template v-else-if="!loading">
          <div v-if="records.length === 0" class="banner">
            暂无换卡记录。完成一笔交换后，可在这里查看并撤销。
          </div>
